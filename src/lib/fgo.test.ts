...
import { uploadInvoiceToFgo, checkFgoStatus } from "./fgo";
  ...
  describe("in sandbox mode", () => {
    ...
    it(
      ...
      withEnv({ EFATURA_ENABLED: "true", FGO_ENV: "sandbox" }, async () => {
        const results = await Promise.all(Array.from({ length: 20 }, () => uploadInvoiceToFgo(mockPayload)));
        const successes = results.filter((r) => r.ok && r.status === "SUBMITTED_OK");
        const errors = results.filter((r) => !r.ok);
        const pending = results.filter((r) => r.ok && r.status === "PENDING_VALIDATION");
