...
import { type ActionState, fromZodError } from "./action-state";
	...
	return async (arg1: unknown, arg2?: unknown): Promise<ActionState> => {
		...
		try {
			...
			if (options.activityLog && user) {
				let diff: Record<string, unknown> | undefined = options.activityLog
					.getDiff
					? (options.activityLog.getDiff(parsedData, result) as
							| Record<string, unknown>
							| undefined)
					: undefined;

				if (diff && options.activityLog.redactFields) {
					diff = redactSensitiveData(
						diff,
						options.activityLog.redactFields,
