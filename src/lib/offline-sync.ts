...
export async function saveTimeEntryOffline(
	entry: Omit<PendingTimeEntry, "id" | "createdAt">,
) {
...
export async function syncPendingTimeEntries(
	actionFn: (formData: FormData) => Promise<ActionState>,
) {
	...
	toast.loading(`Se sincronizează ${entries.length} pontaje...`, {
		id: "sync-toast",
	});
	...
	if (failCount > 0) {
		toast.error(
			`${failCount} pontaje nu au putut fi sincronizate. Vor fi reîncercate ulterior.`,
		);
	}
}
