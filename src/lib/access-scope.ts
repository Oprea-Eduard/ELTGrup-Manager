...
export const resolveAccessScope = cache(
	async (user: AuthUserLike): Promise<AccessScope> => {
		if (
			isPrivilegedUser(user) ||
			user.roleKeys.some((role) => isCompanyWideNonAdminRole(role as RoleKey))
		) {
		...
		const promises: Promise<unknown>[] = [teamProfilePromise];

		// 1. Managed projects (PM/SM)
		if (
			user.roleKeys.includes(RoleKey.PROJECT_MANAGER) ||
			user.roleKeys.includes(RoleKey.SITE_MANAGER)
		) {
			promises.push(
		...
		const teamProfile = results[0] as { teamId: string } | null;
		...
		for (let i = 1; i < results.length; i++) {
			const items = results[i] as Array<{ id?: string; projectId?: string }>;
		...
		if (
			user.roleKeys.includes(RoleKey.PROJECT_MANAGER) ||
			user.roleKeys.includes(RoleKey.SITE_MANAGER) ||
			user.roleKeys.includes(RoleKey.WORKER)
		) {
...
export function inventoryItemScopeWhere(
	user: AuthUserLike,
	scope: AccessScope,
) {
	if (scope.projectIds === null) return {};
...
export async function assertWorkOrderAccess(
	...
	if (scope.projectIds === null) return;
	if (!scope.projectIds.includes(workOrder.projectId))
		throw new Error("Nu ai acces la lucrarea selectata.");

	if (user.roleKeys.includes(RoleKey.WORKER)) {
		const allowed =
			workOrder.responsibleId === user.id ||
			(scope.teamId && workOrder.teamId === scope.teamId);
		if (!allowed) throw new Error("Nu ai acces la lucrarea selectata.");
	}
}

export async function assertSubcontractorAccess(
	user: AuthUserLike,
...
export async function assertEquipmentAccess(
	user: AuthUserLike,
	equipmentId: string,
) {
	const scope = await resolveAccessScope(user);
...
export async function assertInventoryItemAccess(
	user: AuthUserLike,
	...
	if (!exists)
		throw new Error("Nu ai acces la articolul de inventar selectat.");
}

export async function assertInventoryAssignmentAccess(
	user: AuthUserLike,
	assignmentId: string,
) {
	const scope = await resolveAccessScope(user);
