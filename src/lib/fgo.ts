...
function config(): FgoConfig {
	...
	return {
		oauthUrl:
			...
			(sandbox
				? "https://sandbox.anaf.ro/oauth"
				: "https://logincert.anaf.ro/oauth"),
		apiUrl:
...
function sandboxInvoiceResponse(_payload: FgoInvoicePayload): FgoResult {
	const roll = Math.random();
	if (roll < 0.6) {
		// 60% success
		return ok(
			`SANDBOX-${Date.now()}-${Math.floor(Math.random() * 100000)}`,
			"SUBMITTED_OK",
		);
	}
	if (roll < 0.8) {
		...
		return err(
			"VALIDATION_ERRORS",
			"VALIDATION_001",
			"CIF client invalid in baza ANAF",
		);
	}
	if (roll < 0.9) {
		// 10% pending
	...
	return err(
		"REJECTED",
		"REJECTION_001",
		"Factura respinsa: data emiterii incorecta",
	);
}
...
export async function uploadInvoiceToFgo(
	payload: FgoInvoicePayload,
): Promise<FgoResult> {
	if (!isEnabled()) {
		return err(
			"DRAFT_UPLOADED",
			"FGO_DISABLED",
			"eFactura este dezactivata in .env",
		);
	}
	...
	if (!token) {
		return err(
			"DRAFT_UPLOADED",
			"OAUTH_FAILED",
			"Nu s-a putut obtine token de acces FGO",
		);
	}

	const body =
		process.env.FGO_FORMAT === "XML"
			? buildAnafXmlPayload(payload)
			: buildJsonPayload(payload);
	const contentType =
		process.env.FGO_FORMAT === "XML" ? "application/xml" : "application/json";
	...
	if (!response.ok) {
		return err(
			...
			"Eroare la transmiterea facturii catre ANAF",
		);
	}
...
export async function checkFgoStatus(trackingId: string): Promise<FgoResult> {
	...
	const response = await fetchWithRetry(
		`${config().apiUrl}/status/${trackingId}`,
		{
	...
	if (!response.ok) {
		return err(
			"SUBMITTED_ERRORS",
			`HTTP_${response.status}`,
			"Eroare la verificarea statusului FGO",
		);
	}

	const rawStatus =
		(response.data.status as string | undefined) ?? "SUBMITTED_OK";
	...
	const mappedStatus: FgoInvoiceStatus = validStatuses.includes(
...
export async function cancelFgoInvoice(trackingId: string): Promise<FgoResult> {
	...
	const response = await fetchWithRetry(
		`${config().apiUrl}/cancel/${trackingId}`,
		{
			method: "POST",
			headers: { Authorization: `Bearer ${token}` },
		},
	...
	if (!response.ok) {
		return err(
			"SUBMITTED_ERRORS",
			`HTTP_${response.status}`,
			"Eroare la anularea facturii FGO",
		);
	}
