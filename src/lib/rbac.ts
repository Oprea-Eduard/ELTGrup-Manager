...
const rolePermissionOverviews: Record<
	RoleKey,
	Pick<RolePermissionOverview, "summary" | "restrictions">
> = {
	SUPER_ADMIN: {
		summary: "Acces complet la toate modulele si actiunile.",
		restrictions:
			"Poate administra utilizatori, setari si intregul flux operational.",
	},
	ADMINISTRATOR: {
		summary: "Acces complet la toate modulele si actiunile.",
	...
	MAGAZIONER: {
		...
		restrictions:
			"Are acces complet la sectiunea de materiale/scule, dar limitat la vizualizare in rest.",
	},
	PROJECT_MANAGER: {
		summary: "Coordoneaza proiecte, lucrari, echipe, pontaj si rapoarte.",
		restrictions:
			"Nu are acces la facturi, setari sau administrare conturi. Poate aproba cereri de materiale.",
	},
	SITE_MANAGER: {
		summary:
			"Coordoneaza santierul: lucrari, pontaj, materiale, documente si rapoarte.",
		restrictions:
			"Nu are acces la setari, utilizatori, facturi sau oferte. Solicita materiale fara a aproba.",
	},
	BACKOFFICE: {
		summary:
			"Sprijina operatiunile interne: proiecte, lucrari, echipe, pontaj, materiale, documente si rapoarte.",
		restrictions: "Nu are acces la setari, utilizatori sau facturi.",
	},
	WORKER: {
		summary:
			"Lucreaza pe teren: taskuri, pontaj, materiale, documente si rapoarte limitate.",
	...
	CLIENT_VIEWER: {
		summary:
			"Are vizibilitate doar pe proiecte, documente, facturi si rapoarte.",
...
export function getRolePermissionOverview(
	roleKey: RoleKey,
	...
	const resourceSummaries = Object.entries(matrix)
		.map(([resource, actions]) => {
			...
			return {
				...
				actionLabels: typedActions.map(
					(action) => permissionActionLabels[action],
				),
			};
		})
	...
	return {
		...
		isFullAccess:
			roleKey === RoleKey.SUPER_ADMIN || roleKey === RoleKey.ADMINISTRATOR,
		resourceSummaries,
	};
}
...
export function hasPermission(
	...
	return normalizedRoles.some((role) =>
		rolePermissionMatrix[role]?.[resource]?.includes(action),
	);
}
