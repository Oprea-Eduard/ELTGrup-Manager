...
describe("export route ordering", () => {
	beforeEach(() => {
		...
		mockResolveAccessScope.mockResolvedValue({
			projectIds: null,
			teamId: null,
		});
	...
	it("uses deterministic order for pontaj export", async () => {
		const { GET: getPontajExport } = await import(
			"../../app/api/export/pontaj/route"
		);
		const request = mockNextRequest(
			"http://localhost/api/export/pontaj?month=1&year=2026",
		);
	...
	it("uses deterministic order for financiar export", async () => {
		const { GET: getFinanciarExport } = await import(
			"../../app/api/export/financiar/route"
		);
	...
	it("uses deterministic order for materiale export", async () => {
		const { GET: getMaterialeExport } = await import(
			"../../app/api/export/materiale/route"
		);
	...
	it("uses deterministic order for rapoarte export", async () => {
		const { GET: getRapoarteExport } = await import(
			"../../app/api/export/rapoarte/route"
		);
