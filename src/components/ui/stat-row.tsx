...
export function StatRow({
  ...
  return (
    <div className={cn("flex items-center justify-between py-1.5 text-sm", className)}>
      ...
      <span className={cn("font-semibold tabular-nums", muted ? "text-[var(--muted)]" : "text-[var(--foreground)]")}>
        {value}
        {trend === "up" && <span className="ml-1 text-xs text-[var(--status-active)]">↑</span>}
        {trend === "down" && <span className="ml-1 text-xs text-[var(--status-blocked)]">↓</span>}
      </span>
    </div>
  );
}
