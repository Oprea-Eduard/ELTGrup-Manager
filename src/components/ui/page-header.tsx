...
export function PageHeader({
  ...
  return (
    <header
      className={cn(
        "relative overflow-hidden rounded-2xl bg-[linear-gradient(180deg,color-mix(in oklab,var(--surface) 95%,transparent),color-mix(in oklab,var(--surface) 95%,transparent))] px-4 py-4 sm:px-5",
        borderless ? "" : "border border-[var(--border)] shadow-[var(--shadow-panel)]",
      )}
    >
      ...
      <div className="relative space-y-1">
        <h1 className="text-[1.3rem] font-semibold tracking-tight text-[var(--foreground)] sm:text-[1.6rem] lg:text-[1.85rem]">{title}</h1>
