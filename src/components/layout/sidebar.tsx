...
export function Sidebar({
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className={cn("border-b border-[var(--border)]", collapsed ? "px-2 py-3" : "px-4 py-4")}>
        {collapsed ? (
          <div className="flex h-9 w-9 items-center justify-center mx-auto rounded-[var(--radius-md)] bg-[var(--accent)] text-white text-xs font-bold">
            EG
          </div>
        ) : (
          <img
        ...
        {navSections.map((section) => {
          ...
          return (
            <section key={section} className={cn("mb-4", collapsed && "mb-3")}>
              {!collapsed && (
                <p className="mb-1.5 px-2.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[var(--muted)]">{section}</p>
              )}
              <div className="space-y-0.5">
                {sectionItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {active && !collapsed ? <span className="absolute left-0 top-1/2 h-5 w-[2px] -translate-y-1/2 rounded-r-full bg-[var(--accent)]" /> : null}
                      {!item.sub && (
                        <Icon className={cn("h-4 w-4 shrink-0", active ? "text-[var(--accent)]" : "text-[var(--muted)] group-hover:text-[var(--accent)]")} />
                      )}
                      {!collapsed && (
        ...
        <div className="flex items-center gap-2">
          {!collapsed && (userName || userAvatar) && (
            <div className="min-w-0 flex-1 flex items-center gap-2">
              {userAvatar ? (
                <img src={userAvatar} alt="" className="h-7 w-7 shrink-0 rounded-full object-cover" />
              ) : userName ? (
          ...
          <button
            onClick={toggle}
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface-1)] text-[var(--muted-strong)] transition-colors hover:bg-[var(--surface-2)] hover:text-[var(--foreground)]"
