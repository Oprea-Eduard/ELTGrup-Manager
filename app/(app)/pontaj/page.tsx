...
> = {
	DRAFT: {
		label: "Draft",
		tone: "neutral",
		description: "Inregistrare nefinalizata",
	},
	SUBMITTED: {
		label: "Asteapta aprobare",
		tone: "warning",
		description: "Trimis la verificare",
	},
	APPROVED: {
		label: "Aprobat",
		tone: "success",
		description: "Validat pentru salarizare",
	},
	REJECTED: {
		label: "Respins",
		tone: "danger",
		description: "Cerere respinsa",
	},
};
...
export default async function PontajPage({
	...
	const statusFilter = parseEnumParam(
		params.status,
		Object.values(TimeEntryStatus),
	);
	const fromDate = parseDateParam(params.from);
	const toDate = params.to
		? parseDateParam(`${params.to}T23:59:59`)
		: undefined;
	const startAtFilter =
	...
	const submittedEntries = entries.filter(
		(item) => item.status === TimeEntryStatus.SUBMITTED,
	);
	...
	return (
		<PermissionGuard resource="TIME_TRACKING" action="VIEW">
			<div className="page-stack">
				...
				{/* Inline filters */}
				<form
					className="grid gap-2 sm:grid-cols-[auto_auto_auto_auto_auto]"
					method="get"
				>
					...
					>
						...
						{Object.values(TimeEntryStatus).map((status) => (
							<option key={status} value={status}>
								{getTimeEntryStatusMeta(status).label}
							</option>
						))}
					</select>
						...
						<details>
							...
							<form action={bulkTimeEntriesAction} className="mt-3 space-y-3">
								<div className="grid gap-2 md:grid-cols-3">
									...
									<ConfirmSubmitButton
										text="Executa"
										confirmMessage="Confirmi actiunea bulk pentru pontajele selectate?"
									/>
								</div>
				...
				{entries.length === 0 ? (
					<EmptyState
						title="Nu exista pontaj"
						description="Adauga prima inregistrare de timp sau schimba filtrele."
					/>
				) : (
					<>
						{/* Mobile cards */}
					...
					<div className="flex w-full gap-2 sm:w-auto">
						{currentPage > 1 && (
							<Link
								...
								href={buildPontajHref(currentPage - 1, {
