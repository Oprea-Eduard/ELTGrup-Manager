...
import { AssignmentStatus, SubcontractorApprovalStatus } from "@prisma/client";
    ...
    <PermissionGuard resource="TASKS" action="VIEW">
      <div className="page-stack">
        <PageHeader
          ...
          subtitle={`${subcontractor.cui ? `CUI: ${subcontractor.cui} · ` : ""}${meta.label}`}
          actions={
            ...
            {canUpdate && !isArchived ? (
              <form action={updateSubcontractorAction} className="grid gap-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input name="contactName" defaultValue={subcontractor.contactName || ""} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-semibold uppercase tracking-wider text-[var(--muted)]">
                      Email
                    </label>
                    <Input name="email" type="email" defaultValue={subcontractor.email || ""} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-semibold uppercase tracking-wider text-[var(--muted)]">
                      Telefon
                ...
                <div className="flex justify-end">
                  <Button type="submit" size="sm">Salveaza modificarile</Button>
                </div>
              </form>
            ) : (
              <div className="grid gap-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-[var(--foreground)]">{subcontractor.name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--muted)]">CUI</span>
                  <span className="text-[var(--foreground)]">{subcontractor.cui || "—"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--muted)]">Contact</span>
                  <span className="text-[var(--foreground)]">{subcontractor.contactName || "—"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--muted)]">Email</span>
                  <span className="text-[var(--foreground)]">{subcontractor.email || "—"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--muted)]">Telefon</span>
                  <span className="text-[var(--foreground)]">{subcontractor.phone || "—"}</span>
                </div>
              </div>
            )}
            {subcontractor.notes && (
              <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-3">
                ...
                <p className="mt-1 text-sm text-[var(--muted-strong)] whitespace-pre-line">{subcontractor.notes}</p>
              </div>
            )}
          </Card>
          ...
          <Card className="space-y-3">
            ...
            <div className="grid gap-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-[var(--foreground)]">{formatDate(subcontractor.createdAt)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[var(--muted)]">Ultima actualizare</span>
                <span className="text-[var(--foreground)]">{formatDate(subcontractor.updatedAt)}</span>
              </div>
              {isArchived && (
                <div className="flex justify-between">
                  <span className="text-[var(--muted)]">Arhivat la</span>
                  <span className="text-[var(--danger)]">{formatDate(subcontractor.deletedAt!)}</span>
                </div>
              )}
            </div>
            ...
            {isArchived && (
              <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-3">
                <p className="text-sm text-[var(--muted)]">
                  Subcontractorul este arhivat. Datele sunt disponibile doar in mod read-only.
                </p>
              </div>
            )}
          </Card>
