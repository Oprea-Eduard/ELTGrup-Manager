	...
	return (
		<Card>
			<div className="flex flex-wrap items-center gap-2">
				<Badge tone="neutral">{filters.totalClients} clienti gasiti</Badge>
				{filters.hasFilters ? (
			...
			<div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1.6fr)_minmax(220px,0.85fr)_minmax(200px,0.85fr)_auto]">
				...
				>
					...
					<select
						name="type"
						defaultValue={filters.typeFilter || ""}
					...
					<select
						name="archived"
						defaultValue={filters.archivedFilter}
						className="h-11 rounded-lg border border-[var(--border)] bg-[var(--surface-card)] px-3 text-sm text-[var(--foreground)]"
					>
			...
			{canCreate && createDialogOpen ? (
				<ClientCreateDialog createHref={createHref} closeHref={closeHref} />
			) : null}
		</Card>
	);
}
				...
				<div className="flex items-start justify-between gap-3 border-b border-[var(--border)] px-4 py-3 sm:px-5">
					<div>
						...
						<p className="mt-1 text-sm text-[var(--muted)]">
							Deschide formularul intr-un dialog ca sa poti ramane pe lista
							curenta si sa revii rapid dupa salvare.
						</p>
					</div>
					<Link href={closeHref}>
					...
					<div className="max-h-36 overflow-y-auto rounded-xl border border-[var(--border)]/70 bg-[var(--surface-card)] p-3">
						{clients.length > 0 ? (
							...
							<p className="text-sm text-[var(--muted)]">
								Nu exista clienti activi pe pagina curenta pentru arhivare.
							</p>
						)}
					</div>
				</form>
		...
		<div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
			{clients.map((client) => (
				<Card key={client.id} className="space-y-3">
					...
					<div className="flex items-start justify-between gap-3">
						<div>
							<Link
								...
								className="text-base font-bold text-[var(--heading)] hover:underline"
							>
								{client.name}
							</Link>
					...
					<p className="text-sm text-[var(--muted-strong)] break-words">
						{client.email || "-"} • {client.phone || "-"}
					</p>
					<p className="text-xs text-[var(--muted)] break-words">
						Adresa: {client.billingAddress || "-"}
					</p>
					...
					{canUpdate && !client.deletedAt ? (
						<form action={addClientNote} className="mt-3 space-y-2">
							...
							<Textarea
								name="note"
								rows={2}
								placeholder="Adauga nota operationala"
							/>
...
export default async function ClientiPage({
	...
	const typeFilter = parseEnumParam(params.type, clientTypeOptions) ?? null;
	...
	const hasFilters = Boolean(
		query || typeFilter || archivedFilter !== "active",
	);
	const activeClientsOnPage = clients.filter((client) => !client.deletedAt);
	const createHref = buildClientiHref({
		page: currentPage,
		q: query,
		type: typeFilter,
	...
	const closeHref = buildClientiHref({
		page: currentPage,
		q: query,
		type: typeFilter,
		archived: archivedFilter,
	...
	return (
		<PermissionGuard resource="PROJECTS" action="VIEW">
			<div className="page-stack">
				...
				/>
				<BulkActionsCard clients={activeClientsOnPage} canDelete={canDelete} />
				{clients.length === 0 ? (
					<EmptyState
						title={
							hasFilters
								? "Nu exista clienti care sa corespunda filtrelor"
								: "Nu exista clienti in aria ta de acces"
						}
						description={
							hasFilters
