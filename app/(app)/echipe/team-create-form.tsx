	...
	return (
		<form action={formAction} className="grid gap-3 md:grid-cols-2">
			...
			<fieldset className="rounded-xl border border-[var(--border)] bg-[var(--surface-card)] p-3 md:col-span-2">
				<legend className="px-1 text-xs font-semibold uppercase tracking-[0.1em] text-[var(--muted)]">
					Membri
				</legend>
				{workers.length ? (
					<div className="mt-2 grid max-h-56 gap-2 overflow-y-auto pr-1 md:grid-cols-2">
						{workers.map((worker) => (
							<label
								key={worker.id}
								className={`flex items-start gap-2 rounded-lg border px-3 py-2 text-sm text-[var(--foreground)] ${!worker.teamName ? "border-[var(--success)]/20 bg-[var(--success)]/5" : "border-[var(--border)] bg-[var(--surface)]/20"}`}
							>
								<input
									type="checkbox"
									name="memberIds"
