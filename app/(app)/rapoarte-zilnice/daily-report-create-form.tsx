...
export function DailyReportCreateForm({
	...
	return (
		...
		>
			{projects.length === 0 ? (
				<p className="md:col-span-2 xl:col-span-4 text-sm text-[var(--muted)]">
					Nu exista proiecte disponibile. Pentru a adauga un raport, trebuie sa
					existe cel putin un proiect activ.
				</p>
			) : null}
			<select
				name="projectId"
			...
			<div>
				...
				/>
				{state.errors?.workersCount ? (
					<p className="mt-1 text-xs text-[var(--danger)]">
						{state.errors.workersCount[0]}
					</p>
				) : null}
			</div>
			<Input
				name="subcontractorsPresent"
				placeholder="Subcontractori prezenti"
			/>
			...
			<div className="md:col-span-2 xl:col-span-4">
				<Textarea
					...
					rows={2}
					required
					placeholder="Lucrari finalizate azi"
				/>
			</div>
			<div className="md:col-span-2 xl:col-span-2">
				<Textarea name="blockers" rows={2} placeholder="Blocaje / intarzieri" />
			</div>
			<div className="md:col-span-2 xl:col-span-2">
				<Textarea name="safetyIncidents" rows={2} placeholder="Incidente SSM" />
			</div>
			<Input
				name="signatures"
				placeholder="Semnaturi"
				className="md:col-span-2"
			/>
			<Input
				name="photos"
				placeholder="Lista foto (URL-uri separate cu virgula)"
				className="md:col-span-2"
			/>
			<div className="md:col-span-2 xl:col-span-4 flex justify-end">
				<Button type="submit" disabled={pending}>
					{pending ? "Se salveaza..." : "Salveaza raport"}
				</Button>
			</div>
			{state.errors?.workCompleted ? (
				<p className="md:col-span-2 xl:col-span-4 text-xs text-[var(--danger)]">
					{state.errors.workCompleted[0]}
				</p>
			) : null}
		</form>
	);
