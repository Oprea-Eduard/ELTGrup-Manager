...
function revalidateDailyReportsPaths(input?: {
...
export async function createDailyReportAction(
	...
	try {
		...
		return {
			ok: false,
			message:
				error instanceof Error ? error.message : "Eroare la salvare raport",
		};
	}
}
...
export async function deleteDailyReport(formData: FormData) {
	...
	if (!parsed.success)
		throw new Error("Raport zilnic invalid pentru stergere.");
	...
	if (!report)
		throw new Error("Raportul zilnic nu exista sau a fost deja sters.");
...
export async function bulkDeleteDailyReports(formData: FormData) {
	...
	if (!parsed.success)
		throw new Error("Selectie bulk invalida pentru rapoarte zilnice.");
	...
	if (reports.length === 0)
		throw new Error("Nu am gasit rapoartele selectate pentru stergere.");
	...
	const deletableReports =
		scope.projectIds === null
			...
			: reports.filter((report) =>
					scope.projectIds?.includes(report.projectId),
				);
	...
	const projectIds = [
		...new Set(deletableReports.map((report) => report.projectId)),
	];
	const workOrderIds = [
		...new Set(
			deletableReports
				.map((report) => report.workOrderId)
