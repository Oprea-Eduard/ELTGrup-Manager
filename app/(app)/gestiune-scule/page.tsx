...
const availabilityFilterValues = [
	"all",
	"available",
	"assigned",
	"unassigned",
	...
	return (
		<section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
			...
			<Card>
				...
				<p className="mt-1 text-xs text-[var(--muted)]">
					pe pagina curenta: {rows.filter((r) => r.lowStock).length}
				</p>
			</Card>
			<Card>
				<p className="text-[11px] uppercase tracking-[0.1em] text-[var(--muted)]">
				...
				<p className="mt-1 text-xs text-[var(--muted)]">
					expirari sau verificari in 7 zile
				</p>
			</Card>
		</section>
	);
		...
		<Card>
			<form className="grid gap-3 md:grid-cols-[minmax(0,1.3fr)_minmax(180px,0.8fr)_minmax(170px,0.75fr)_minmax(120px,0.45fr)_auto]">
				<Input
					...
					placeholder="Cauta dupa nume, cod intern, serie, brand"
				/>
				<select
					name="categoryId"
					defaultValue={params.categoryId || ""}
					className="h-11 rounded-lg border border-[var(--border)] bg-[var(--surface-card)] px-3 text-sm"
				>
...
function InventoryDesktopTable({
	...
	rows: RowItem[];
	canManage: boolean;
	inventoryItemStatusLabels: Record<string, string>;
}) {
	return (
						...
						<tr key={row.item.id}>
							<TD>
								<div className="space-y-1">
									<Link
										href={`/gestiune-scule/${row.item.id}`}
							...
							<TD>
								<p className="font-semibold text-[var(--foreground)]">
									...
									{formatQty(row.quantityTotal)} {row.item.unitOfMeasure}
								</p>
								<p className="text-xs text-[var(--muted)]">
									Minim:{" "}
									{row.item.minimumStock !== null
							...
							<TD>
								{row.activeCount > 0 ? (
									<div className="space-y-1">
										...
										<p className="text-xs text-[var(--muted)]">
											{row.activeCount} alocari active
										</p>
									</div>
								) : (
							...
							<TD>
								<div className="space-y-1 text-xs">
									...
									<p className="text-[var(--muted)]">
										Inspectie:{" "}
										{row.item.nextInspectionDate
											? formatDate(row.item.nextInspectionDate)
											: "-"}
									</p>
									<div className="flex flex-wrap gap-1.5">
										{row.lowStock ? (
											<Badge tone="warning">Stoc mic</Badge>
										) : null}
										{row.expired ? <Badge tone="danger">Expirat</Badge> : null}
										{!row.expired && row.inspectionSoon ? (
											<Badge tone="warning">Inspectie curand</Badge>
										) : null}
									</div>
								</div>
							</TD>
							<TD>
								<Badge
									tone={statusTone[row.item.status as InventoryItemStatus]}
								>
									{
										inventoryItemStatusLabels[
											row.item.status as InventoryItemStatus
										]
									}
								</Badge>
							</TD>
								...
								<div className="grid min-w-[200px] gap-2">
									...
									{canManage && (
										<form
											...
											className="rounded-lg border border-[var(--border)]/70 bg-[var(--surface-2)] p-2"
										>
											...
											<label className="grid gap-1">
												...
												<select
													name="status"
													defaultValue={row.item.status}
													className="h-9 min-w-0 rounded-md border border-[var(--border)] bg-[var(--surface-card)] px-2 text-xs"
												>
													{Object.values(InventoryItemStatus).map((s) => (
														<option key={s} value={s}>
															{inventoryItemStatusLabels[s]}
														</option>
													))}
												</select>
											</label>
			...
			{rows.map((row) => (
				...
				>
					<div className="flex items-start justify-between gap-2">
						...
						<Badge tone={statusTone[row.item.status as InventoryItemStatus]}>
							{
								inventoryItemStatusLabels[
									row.item.status as InventoryItemStatus
								]
					...
					<div className="mt-2 space-y-1 text-xs text-[var(--muted)]">
						<p>
							Stoc: {formatQty(row.quantityAvailable)} /{" "}
							{formatQty(row.quantityTotal)} {row.item.unitOfMeasure}
						</p>
						<p>Categorie: {row.item.category?.name || "Fara categorie"}</p>
						<p>
							Depozit: {row.item.warehouse.name}
							{row.item.location ? ` • ${row.item.location.name}` : ""}
						</p>
						...
						{canManage && (
							<form action={updateInventoryItemStatusAction} className="flex-1">
								...
								<input
									type="hidden"
									name="status"
									value={InventoryItemStatus.AVAILABLE}
								/>
...
export default async function GestiuneSculePage({
	...
	const availability =
		parseEnumParam(params.availability, availabilityFilterValues) || "all";
	...
	if (availability === "assigned")
		where.assignments = { some: { status: { in: assignmentStatuses } } };
	if (availability === "unassigned")
		where.assignments = { none: { status: { in: assignmentStatuses } } };
	...
	const [
		...
		prisma.user.findMany({
			where: {
				isActive: true,
				deletedAt: null,
				roles: { some: { role: { key: { notIn: [RoleKey.CLIENT_VIEWER] } } } },
			},
	...
	const processedQtyMap = new Map<string, number>(
		processedByAssignment.map((row) => [
			row.assignmentId ?? "",
			Number(row._sum.quantity || 0),
		]),
	);
	const activeAssignmentsByItem = new Map<string, object[]>();
	const returnAssignmentOptions = activeAssignments.flatMap((assignment) => {
		...
		const group = activeAssignmentsByItem.get(assignment.itemId);
	...
	const now = new Date();
	const rows: RowItem[] = inventoryItems.map((item) => {
		const activeRows = (activeAssignmentsByItem.get(item.id) || []) as {
			issuedToUser: { firstName: string; lastName: string };
		}[];
		const activeCount = activeRows.length;
		const firstHolder = activeRows[0]
			? personName(activeRows[0].issuedToUser)
			: null;
		const quantityAvailable = Number(item.quantityAvailable);
		const quantityTotal = Number(item.quantityTotal);
		const minimumStock = Number(item.minimumStock || 0);
		const lowStock =
			item.minimumStock !== null && quantityAvailable <= minimumStock;
	...
	const inspectionAlerts = rows.filter(
		(row) => row.inspectionSoon || row.expired,
	).length;
	const assignedCount = rows.filter((row) => row.activeCount > 0).length;
	const firstVisibleItem =
		filteredItems === 0 || rows.length === 0 ? 0 : skip + 1;
				...
				<Card className="p-0">
					<div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)]/70 px-4 py-3">
						<div>
							...
							<p className="text-xs text-[var(--muted)]">
								{firstVisibleItem}-{lastVisibleItem} din {filteredItems}{" "}
								rezultate filtrate
							</p>
						</div>
						<div className="flex items-center gap-2 text-xs text-[var(--muted-strong)]">
							...
							<span>
								Pagina {boundedCurrentPage} din {totalPages}
							</span>
							<Link
					...
					<div className="max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-[var(--border-strong)] scrollbar-track-transparent">
						<InventoryDesktopTable
							rows={rows}
							canManage={canManage}
							inventoryItemStatusLabels={inventoryItemStatusLabels}
						/>
						<InventoryMobileCards rows={rows} canManage={canManage} />
					</div>
				</Card>
				{canCreate ? (
					<section className="grid items-start gap-4 xl:grid-cols-2">
						<details
							className="rounded-2xl border border-[var(--border)]/80 bg-[var(--surface-2)] p-4"
							open
						>
							...
							<div className="mt-4">
								...
								<InventoryItemForm
									...
									locations={locations.map((l) => ({
										id: l.id,
										label: `${l.warehouse.name} • ${l.name} (${l.code})`,
									}))}
								/>
							</div>
						</details>
				...
				{canManage ? (
					<section className="grid items-start gap-4 xl:grid-cols-2">
									...
									items={rows.flatMap((row) =>
										row.quantityAvailable > 0
											? [
													{
														id: row.item.id,
														label: `${row.item.internalCode} • ${row.item.name}`,
													},
												]
											: [],
									)}
						...
						<details className="rounded-2xl border border-[var(--warning)]/55 bg-[var(--warning)]/10 p-4">
							<summary className="cursor-pointer list-none text-sm font-semibold text-[var(--warning)]">
								Corectie inventar
							</summary>
							<div className="mt-4">
								<p className="text-[11px] uppercase tracking-[0.1em] text-[var(--muted)]">
								...
								<h2 className="mt-1 text-lg font-semibold text-[var(--foreground)]">
									Ajusteaza stoc cu motiv
								</h2>
								<p className="mt-2 text-xs text-[var(--warning)]">
									Foloseste corectia doar pentru inventar verificat; modifica
									stocul oficial.
								</p>
								<InventoryAdjustmentForm
									items={rows.map((row) => ({
						...
						{recentMovements.map((movement) => (
							...
							>
								<div className="flex items-center justify-between gap-2">
									...
									<Badge
										tone={movementTone[movement.type as InventoryMovementType]}
									>
										{movementLabels[movement.type as InventoryMovementType]}
									</Badge>
								</div>
								<p className="mt-1 text-xs text-[var(--muted)]">
									Cantitate {formatQty(Number(movement.quantity))}{" "}
									{movement.item.unitOfMeasure} •{" "}
									{movement.project?.title || "Fara proiect"}
								</p>
								<p className="text-xs text-[var(--muted)]">
