...
export function InventoryCategoryForm() {
	...
	return (
		<form action={formAction} className="mt-3 grid gap-3 md:grid-cols-2">
			...
			<Field
				label="Descriere"
				hint="Optional: ce include categoria."
				className="md:col-span-2"
			>
				<Input
					name="description"
					placeholder="Rotopercutoare, slefuitoare, autofiletante"
				/>
			</Field>
...
export function InventoryLocationForm({
	...
	return (
		<form action={formAction} className="mt-3 grid gap-3 md:grid-cols-2">
			...
			<Field
				label="Observatii"
				hint="Optional: informatii utile pentru magazioner."
				className="md:col-span-2"
			>
...
export function InventoryItemForm({
	...
	return (
		...
		>
			...
			<Field
				label="Tip articol"
				hint="Scula, echipament, consumabil sau articol de stoc."
			>
				<select
					name="itemType"
					defaultValue={InventoryItemType.TOOL}
					className={selectClassName}
				>
			...
			<Field
				...
				hint="Cantitatea fizica existenta in companie."
			>
				<Input
					name="quantityTotal"
					type="number"
			...
			<Field
				label="Expira la"
				hint="Pentru consumabile/documente de valabilitate."
			>
			...
			<Field
				label="Observatii"
				hint="Detalii utile pentru operare."
			...
			<div className="md:col-span-2 xl:col-span-3 flex justify-end">
				<Button type="submit" disabled={pending}>
					{pending ? "Se salveaza..." : "Adauga articol"}
				</Button>
			</div>
		</form>
	);
		...
		<form action={formAction} className="mt-3 grid gap-3 md:grid-cols-2">
			...
			<Field label="Conditie la predare" hint="Starea articolului la iesire.">
				<select
					...
					defaultValue={InventoryCondition.GOOD}
					className={selectClassName}
				>
			...
			<Field
				label="Observatii"
				hint="Optional: lot, accesorii, instructiuni."
				className="md:col-span-2"
			>
...
export function InventoryReturnForm({
	...
	return (
		<form action={formAction} className="mt-3 grid gap-3 md:grid-cols-2">
			...
			<Field
				label="Conditie la retur"
				hint="Starea fizica la primire in depozit."
			>
				<select
			...
			<div className="grid gap-2 rounded-lg border border-[var(--border)] bg-[var(--surface-card)] px-3 py-2 text-sm text-[var(--muted-strong)]">
				<label className="inline-flex items-center gap-2">
					<input type="checkbox" name="isDamaged" className="size-4" />
					Retur cu defect / necesita service
				</label>
				<label className="inline-flex items-center gap-2">
					<input type="checkbox" name="isLost" className="size-4" />
					Marcat pierdut
				</label>
			</div>
			<Field
				label="Observatii"
				hint="Detalii despre retur, defect sau lipsuri."
				className="md:col-span-2"
			>
...
export function InventoryAdjustmentForm({ items }: { items: Option[] }) {
	...
	return (
		<form action={formAction} className="mt-3 grid gap-3 md:grid-cols-2">
			...
			<Field
				label="Directie corectie"
				hint="Plus pentru adaugare, minus pentru scadere."
			>
			...
			<label className="inline-flex items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--surface-card)] px-3 py-2 text-sm text-[var(--muted-strong)]">
				<input type="checkbox" name="affectTotal" className="size-4" />
				Ajusteaza si cantitatea totala
			</label>
			<Field
				label="Motiv"
				hint="Obligatoriu pentru audit inventar."
				className="md:col-span-2"
			>
			...
			<Field
				label="Observatii"
				hint="Optional: document, persoana, context."
				className="md:col-span-2"
			>
...
export function InventoryInspectionForm({ items }: { items: Option[] }) {
	...
	return (
		<form action={formAction} className="mt-3 grid gap-3 md:grid-cols-2">
			...
			<Field
				label="Tip verificare"
				hint="Inspectie, calibrare, garantie, valabilitate."
			>
				<select
					name="type"
					defaultValue={InventoryInspectionType.INSPECTION}
					className={selectClassName}
				>
			...
			<Field
				label="Rezultat"
				hint="Daca nu trece verificarea, articolul trece in service."
			>
				<select
					name="result"
					defaultValue={InventoryInspectionResult.PASS}
					className={selectClassName}
				>
			...
			<Field
				label="Observatii"
				hint="Detalii masuratori, constatari, defecte."
				className="md:col-span-2"
			>
				<Input
					name="notes"
					placeholder="Calibrare efectuata de laborator extern"
				/>
			</Field>
			<div className="md:col-span-2 flex justify-end">
				<Button type="submit" variant="secondary" disabled={pending}>
