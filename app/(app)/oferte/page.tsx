...
function OfferFiltersCard({
	...
	statusMap: Map<
		string,
		{ _count: { _all: number }; _sum: { totalAmount: Prisma.Decimal | null } }
	>;
}) {
	return (
		<Card>
				...
				{offerStatusOptions.map((status) => {
					...
					return (
						<Link
							...
							className={`rounded-[var(--radius-sm)] border px-2.5 py-1 text-[11px] font-semibold transition-colors ${isActive ? "border-[var(--accent)] bg-[var(--accent)]/10 text-[var(--accent)]" : "border-[var(--border)] text-[var(--muted-strong)] hover:border-[var(--border-strong)]"}`}
						>
			...
			<form className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1.5fr)_minmax(220px,0.8fr)_auto_auto]">
...
export default async function OfertePage({
	...
	const statusFilter =
		parseEnumParam(params.status, offerStatusOptions) ?? null;
	...
	if (clientIdFilter) andFilters.push({ clientId: clientIdFilter });
	if (query)
		andFilters.push({
			OR: [
