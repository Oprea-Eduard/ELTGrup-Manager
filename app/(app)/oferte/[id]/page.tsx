...
function OfferKpiRow({
	...
	return (
		<section className="page-kpis">
			...
			<KpiCard
				label="Valabilitate"
				value={isExpired ? "Expirata" : `${daysUntilExpiry} zile`}
				helper={formatDate(offer.validUntil)}
				severity={
					isExpired ? "blocked" : daysUntilExpiry <= 7 ? "pending" : "active"
				}
			/>
			<KpiCard
...
function OfferDetailCard({ offer }: { offer: OfferDetail }) {
	const meta = offerStatusMeta[offer.status];
	return (
...
function OfferClientCard({
	...
	const _meta = offerStatusMeta[offer.status];
	const now = new Date();
	const daysUntilExpiry = Math.ceil(
		(offer.validUntil.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
	);
	const _isExpired = daysUntilExpiry < 0;
	return (
		<Card className="space-y-3">
			<p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--muted)]">
				Client
				...
				<Table>
					...
					<tbody>
						...
						<tr className="border-t-2 border-[var(--border)]">
							<td
								colSpan={5}
								className="px-4 py-3 text-right text-sm font-semibold text-[var(--foreground)]"
							>
								Total pozitii
							</td>
		...
		<PermissionGuard resource="OFFERS" action="VIEW">
			<div className="page-stack">
				<PageHeader
					...
					actions={<OfferHeaderActions offer={offer} canUpdate={canUpdate} />}
				/>
				<OfferKpiRow
					offer={offer}
					isExpired={isExpired}
				...
				<OfferItemsTable offer={offer} itemsTotal={itemsTotal} />
			</div>
		</PermissionGuard>
	);
}
