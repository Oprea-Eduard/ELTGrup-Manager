...
export function OfferCreateForm({ clients }: { clients: ClientOption[] }) {
	...
	useEffect(() => {
		if (state.ok && state.message) {
			...
			const id = setTimeout(() => (window.location.href = "/oferte"), 800);
		...
		<form action={formAction} className="space-y-6">
			<Card className="space-y-4 p-5">
				...
				<div className="grid gap-4 md:grid-cols-2">
					<div className="space-y-1.5">
						...
						<Input
							name="title"
							placeholder="Ex: Instalatii electrice + PSI - Hala SIGEMO"
							required
						/>
						{state.errors?.title ? (
				...
				<div className="space-y-1.5">
					...
					<textarea
						...
						placeholder="Detalii tehnice, domeniu de aplicare..."
					/>
				</div>
			...
			<Card className="space-y-4 p-5">
				...
				{items.map((item, idx) => (
					<div
						key={item.id}
						className="grid gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-4 md:grid-cols-[1fr_100px_120px_40px]"
					>
						<div className="space-y-1.5">
						...
						<div className="flex items-end">
							{items.length > 1 && (
								<button
									...
									className="size-10 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--danger)] transition hover:bg-[var(--danger)]/10"
								>
									×
								</button>
							)}
