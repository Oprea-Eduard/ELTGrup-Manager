...
export default async function NotificariPage({
	...
	const [notifications, unreadCount, typeBreakdown, globalUnreadCount] =
		await Promise.all([
	...
	const byType = typeBreakdown.reduce<Record<string, number>>(
		(acc, notification) => {
	...
	const prevHref =
		currentPage > 1
			? buildNotificariHref({ page: currentPage - 1, q: query, readState })
			: null;
	const nextHref =
			...
			<div className="page-stack">
				...
				<Card>
					<div className="flex flex-wrap items-center gap-2">
						...
						{readCount > 0 ? (
							<Badge tone="success">{readCount} citite</Badge>
						) : null}
					</div>
					<form
						method="get"
						action="/notificari"
						className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1.6fr)_minmax(220px,0.85fr)_auto]"
					>
						<input type="hidden" name="page" value="1" />
						<Input
							name="q"
							defaultValue={query}
							placeholder="Cauta in titlu, mesaj sau link"
						/>
						<select
							name="readState"
				...
				{notifications.length === 0 ? (
					<EmptyState
						title={
							hasFilters
								? "Nu exista notificari pentru filtrele selectate"
									...
									<div className="space-y-2">
										<div className="flex flex-wrap items-center gap-2">
											...
											<Badge
												tone={
													severityTones[
														getNotificationSeverity(notification.type)
													]
												}
											>
												{
											...
											<Badge tone={getReadStateTone(notification.isRead)}>
												{notification.isRead ? "Citita" : "Necitita"}
											</Badge>
										</div>
										<div>
