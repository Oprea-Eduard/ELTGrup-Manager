...
export function ProjectInstallations({
	...
	return (
		<div suppressHydrationWarning className="mt-3 space-y-1">
			...
			{installations.map((inst) => {
				const isOverdue =
					now && inst.nextCheckAt && inst.nextCheckAt.getTime() < now.getTime();
				return (
					<div
						key={inst.id}
						className="rounded-xl border border-[var(--border)]/70 bg-[var(--surface)] p-3 text-sm"
					>
						<div className="flex flex-wrap items-center justify-between gap-2">
							...
							<Badge
								tone={
									inst.status === InstallationStatus.CERTIFIED
										? "success"
										: inst.status === InstallationStatus.INSTALLED
											? "info"
											: inst.status === InstallationStatus.UNDER_TEST
												? "warning"
												: inst.status === InstallationStatus.UNDER_MAINTENANCE
													? "danger"
													: "neutral"
								}
							>
								{inst.status === InstallationStatus.UNDER_TEST
									...
									: inst.status === InstallationStatus.CERTIFIED
										...
										: inst.status === InstallationStatus.UNDER_MAINTENANCE
											? "In service"
											: inst.status === InstallationStatus.DECOMMISSIONED
												? "Retras"
												: "Instalat"}
							</Badge>
						</div>
						<div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-[var(--muted)]">
							{inst.warrantyMonths ? (
								<span>Garantie {inst.warrantyMonths} luni</span>
							) : null}
							{inst.installedAt ? (
								<span>Instalat {formatDate(inst.installedAt)}</span>
							) : null}
							{inst.nextCheckAt ? (
								<span
									className={
...
function InstallationStatusButtons({
	installation,
	projectId,
}: {
