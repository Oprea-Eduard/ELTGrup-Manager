		...
		<Card>
			<div className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--border)] pb-3">
				<div>
					...
					<p className="mt-1 text-sm text-[var(--muted)]">
						Documentele deja incarcate sunt grupate automat pe discipline.
						Adauga tag-ul disciplinei (ex.{" "}
						<span className="font-semibold text-[var(--foreground)]">
							{planGroups[0]?.sampleTag}
						</span>
						) pentru clasificare corecta.
					</p>
				</div>
			...
			<div className="mt-4 grid gap-3 xl:grid-cols-2">
				{planGroups.map((group) => (
					<div
						key={group.key}
						className="rounded-2xl border border-[var(--border)] bg-[var(--surface-card)] p-4 shadow-[var(--shadow-panel)]"
					>
						<div className="flex items-start justify-between gap-3">
							...
							<Badge tone={group.docs.length ? "success" : "neutral"}>
								{group.docs.length}
							</Badge>
						</div>
						...
						{group.docs.length > 0 ? (
							<div className="mt-3 space-y-2">
											...
											<div className="min-w-0 space-y-1">
												...
												<p className="text-xs text-[var(--muted)]">
													...
													{document.project?.title ||
														document.workOrder?.title ||
														"Document"}
												</p>
											</div>
											<Badge tone={document.isPrivate ? "neutral" : "success"}>
												{document.isPrivate ? "Privat" : "Public"}
											</Badge>
										</div>
								...
								{group.docs.length > 3 ? (
									<p className="text-xs text-[var(--muted)]">
										+ {group.docs.length - 3} alte documente in aceasta
										categorie.
									</p>
								) : null}
							</div>
							...
							<div className="mt-3 rounded-xl border border-dashed border-[var(--border-strong)] bg-[var(--surface)] p-3 text-xs text-[var(--muted)]">
								Nu am gasit documente pentru aceasta disciplina. Adauga tag-ul{" "}
								<span className="font-semibold text-[var(--foreground)]">
									{group.sampleTag}
								</span>{" "}
								sau include disciplina in titlu.
							</div>
						)}
					</div>
				))}
