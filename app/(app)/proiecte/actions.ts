...
async function _getNextProjectCode() {
...
async function archiveProjectsWithWorkOrders(projectIds: string[]) {
	if (projectIds.length === 0)
		return { archivedProjects: 0, archivedWorkOrders: 0 };
	const now = new Date();
...
export async function deleteProject(formData: FormData) {
	...
	await softDeleteProject(id, currentUser.id);
...
export async function bulkProjectsAction(formData: FormData) {
	...
	const parsed = bulkProjectSchema.safeParse({
	...
	const scopedIds =
		scope.projectIds === null
			? parsed.data.ids
			: parsed.data.ids.filter((id) => scope.projectIds?.includes(id));
	if (scopedIds.length === 0)
		throw new Error("Nu ai acces la proiectele selectate.");
	...
	if (parsed.data.operation === "ARCHIVE") {
		...
		for (const project of projectsBefore) {
			...
			notificationsPerManager.set(
				project.managerId,
				(notificationsPerManager.get(project.managerId) || 0) + 1,
			);
		}
		...
		await Promise.all(
			Array.from(notificationsPerManager.entries()).map(([userId, count]) =>
				notifyUser({
					userId,
					type:
						parsed.data.status === ProjectStatus.BLOCKED
