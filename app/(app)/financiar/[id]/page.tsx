...
export default async function FinanciarDetailPage({
  ...
  const isOverdue = invoice.status === "OVERDUE" || (invoice.dueDate < new Date() && invoice.status !== "PAID" && invoice.status !== "CANCELED");
    ...
    <PermissionGuard resource="INVOICES" action="VIEW">
      <div className="page-stack">
        <PageHeader
          ...
          actions={
            <div className="flex flex-wrap gap-2">
              ...
              {canUpdate && invoice.fgoStatus && invoice.fgoStatus !== "SUBMITTED_OK" && invoice.fgoStatus !== "SENT_TO_ANAF" && invoice.fgoStatus !== "SIGNED" && (
                <form action={sendInvoiceToFgo} className="inline">
                  <input type="hidden" name="invoiceId" value={invoice.id} />
        ...
        <section className="page-kpis">
          ...
          <KpiCard
            label="Rest de plata"
            value={formatCurrency(Number(invoice.totalAmount) - Number(invoice.paidAmount))}
            helper={`Achitat: ${formatCurrency(Number(invoice.paidAmount))}`}
            severity={Number(invoice.totalAmount) - Number(invoice.paidAmount) > 0 ? "pending" : "done"}
          />
        </section>
          ...
          <Card className="space-y-3">
            ...
            <div className="grid gap-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-[var(--foreground)]">{invoice.invoiceNumber}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[var(--muted)]">Status</span>
                <Badge tone={invoice.status === "OVERDUE" ? "danger" : invoice.status === "PAID" ? "success" : invoice.status === "CANCELED" ? "neutral" : "warning"}>
                  {invoiceStatusLabels[invoice.status]}
                </Badge>
              </div>
              <div className="flex justify-between">
                <span className="text-[var(--muted)]">Data emitere</span>
                <span className="text-[var(--foreground)]">{formatDate(invoice.issueDate)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[var(--muted)]">Data scadenta</span>
                <span className={`font-medium ${isOverdue ? "text-[var(--danger)]" : "text-[var(--foreground)]"}`}>
              ...
              {invoice.paidAt && (
                <div className="flex justify-between">
                  ...
                  <span className="text-[var(--success)]">{formatDate(invoice.paidAt)}</span>
                </div>
              )}
            </div>
            ...
            <div className="border-t border-[var(--border)] pt-3">
               <div className="flex justify-between items-center">
                 <span className="text-sm text-[var(--muted)]">Sincronizare FGO</span>
            ...
            {invoice.notes && (
              <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-3">
                ...
                <p className="mt-1 text-sm text-[var(--foreground)] whitespace-pre-line">{invoice.notes}</p>
              </div>
            )}
          </Card>
