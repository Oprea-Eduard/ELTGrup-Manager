...
export async function GET(
  ...
  try {
    ...
    if (!document) {
      return NextResponse.json({ error: "Document inexistent.", code: "NOT_FOUND" }, { status: 404 });
    }
    ...
    if (document.isPrivate && hasExternalRole && document.uploadedById !== currentUser.id) {
      return NextResponse.json({ error: "Nu ai acces la acest document privat.", code: "FORBIDDEN" }, { status: 403 });
    }
    ...
    if (error instanceof Error && error.message.includes("nu exista") || error instanceof Error && error.message.includes("ENOENT")) {
      return NextResponse.json({ error: "Fisierul nu mai exista pe disk.", code: "FILE_MISSING" }, { status: 404 });
    }
    ...
    return NextResponse.json({ error: message, code: status === 403 ? "FORBIDDEN" : "INTERNAL_ERROR" }, { status });
  }
}
