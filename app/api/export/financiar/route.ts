...
export async function GET(request: NextRequest) {
	try {
		...
		const data = invoices.map((invoice) => ({
			...
			Rest: (Number(invoice.totalAmount) - Number(invoice.paidAmount)).toFixed(
				2,
			),
		...
		const message =
			error instanceof Error ? error.message : "Eroare la export financiar";
