...
export async function GET(request: NextRequest) {
  try {
    ...
    if (query.data.startDate) {
      where.reportDate = { ...((where.reportDate as object) || {}), gte: new Date(query.data.startDate) };
    }
    if (query.data.endDate) {
      where.reportDate = { ...((where.reportDate as object) || {}), lte: new Date(query.data.endDate) };
    }
    ...
    const message = error instanceof Error ? error.message : "Eroare la export rapoarte";
