...
export async function GET(
  ...
  try {
    ...
    if (!report) {
      return NextResponse.json({ error: "Raport inexistent" }, { status: 404, headers: corsHeaders });
    }

    try {
      await assertProjectAccess(currentUser, report.projectId);
    } catch {
      return NextResponse.json({ error: "Neautorizat" }, { status: 403, headers: corsHeaders });
    }
    ...
    const message = error instanceof Error ? error.message : "Eroare la generarea PDF-ului";
